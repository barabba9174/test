...
import {
    oneOfType, func, node, element, bool, number
} from 'prop-types';
...
export default class Grid extends Component {
    ...
    handleClick = (row, cell) => {
        ...
        ;
        this.setState({ grid: updateGrid }, () => this.startNavigation(updateGrid));
    }
    ...
    render() {
        ...
        return (
            <GridWrapper theme={{ button: 'default' }} colsNumber={cols}>
                {grid.map((row, irow) => (
                    row.map((cell, icell) => <Cell
                            key={`cell${irow}_${icell}`}
                        type={(bestPath.includes(`${irow}_${icell}`)) ? 'path' : cell}
